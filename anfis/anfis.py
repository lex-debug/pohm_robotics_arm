...
import itertools
import numpy as np
# from .membership.mfDerivs import *
    ...
    def trainHybridJangOffLine(self, epochs=5, tolerance=1e-5, initialGamma=1000, k=0.01):
        ...
        while (epoch < epochs) and (convergence is not True):
            ...
            if len(self.errors) >= 5:
                if (self.errors[-1] < self.errors[-2]) and (self.errors[-2] < self.errors[-3]) and (self.errors[-3] < self.errors[-4]) and (self.errors[-5] > self.errors[-4]):
                    k = k * 0.9
